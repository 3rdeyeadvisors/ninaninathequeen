...
const ShippingUpdateEmail = ({ customerName = '', orderId = '', status = '', trackingNumber }: ShippingUpdateProps) => {
  const statusMsg = statusMessages[status] || `Your order status has been updated to: ${status}`
  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>Order Update — {status}</Preview>
      <Body style={main}>
        <Container style={container}>
          <Text style={logo}>{SITE_NAME}</Text>
          <Hr style={divider} />
          <Heading style={h1}>Shipping Update</Heading>
          <Text style={text}>{customerName || 'there'}, {statusMsg}</Text>
          <Section style={card}>
            <Text style={{ ...text, margin: '0 0 8px 0' }}><strong style={{ color: '#C9A96E' }}>Order:</strong> {orderId.slice(0, 8).toUpperCase()}</Text>
            <Text style={{ ...text, margin: '0 0 8px 0' }}><strong style={{ color: '#C9A96E' }}>Status:</strong> {status}</Text>
            {trackingNumber && <Text style={{ ...text, margin: '0' }}><strong style={{ color: '#C9A96E' }}>Tracking:</strong> {trackingNumber}</Text>}
          </Section>
          <Section style={btnCenter}>
            <Button href={`${SITE_URL}/account`} style={btn}>View Order Details</Button>
          </Section>
